...
var _storage = multer.diskStorage({
    ...
    cb(null,Date.now()+"."+file.orignalname.split('.').pop());
}
});
...
router. post('/upload', upload.single('userPhoto'),function(req,res){
    var filenmae = req.file.filename;
    ...
    connection.query('select * from file where id = ?;', [req.params.file_id], function(error,cursor){
        if(error != undefined){
            ...
            if(cursor.length == undefined || cursor.length < 1)
                res.sendStatus(404);
            elseres.json(cursor[0]);
        }
    });
});

router.get('/uploads/:file_name',function(req,res,net){
    connection.query('select = from file where filename = ?;', [req.params.file.name], function(error,cursor){
        if (error !=undefined){
            res.sendStatus(501);
        }
        else{
